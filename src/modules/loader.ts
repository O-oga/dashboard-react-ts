...
export const sendToHA = (
  ...
  return new Promise<IncomingMessage>((resolve, reject) => {
    ...
    if (connection.readyState !== WebSocket.OPEN) {
      ...
      reject(
        new Error(
          `WebSocket is not ready. Current state: ${connection.readyState} (${stateNames[connection.readyState] || 'UNKNOWN'})`
        )
      )
      return
    }
    ...
    if (data.type !== 'auth' && 'id' in data) {
      pendingRequests.set(data.id, {
        ...
        reject: reject as (error: Error | unknown) => void,
      })
    }
  ...
  return new Promise((resolve, reject) => {
    ...
    const checkReady = () => {
      ...
      if (connection.readyState === WebSocket.OPEN) {
        ...
        reject(
          new Error(
            `Timeout waiting for WebSocket connection. Current state: ${connection.readyState}`
          )
        )
      } else {
...
const sendAuthMessage = async (token: string): Promise<void> => {
  try {
    ...
    return new Promise<void>((resolve, reject) => {
      ...
      const newAuthMessage = { ...messages.auth }
...
export const createConnection = (
  ...
  return new Promise((resolve, reject) => {
    ...
    if (connection && connection.readyState === WebSocket.OPEN) {
      console.log(
        'Connection already exists and is open, reusing existing connection'
      )
      resolve(connection)
      return
    }
    ...
    if (connection && connection.readyState === WebSocket.CONNECTING) {
      console.log(
        'Connection is being established, waiting for it to complete...'
      )
...
const createNewConnection = (
    ...
    sendAuthMessage(token)
      .then(() => {
        ...
        createEntitysStateList()
          ...
          .catch(error => {
            console.error(
              'Failed to create entity state list during connection setup:',
              error
            )
          })
        resolve(connection)
  ...
  connection.onclose = event => {
    ...
    if (!isResolved && event.code !== 1000) {
      ...
      console.log(
        'Connection closed after authentication, attempting to reconnect...'
      )
      isAuthenticated = false
      const newAuthData = getAuthData()
      if (newAuthData) {
...
export const changeDeviceState = async (
  ...
  if (serverEvent) {
    ...
    const newMessage: ChangeStateMessage = { ...messages.changeState }
...
export const createEntitysStateList = async () => {
  try {
    ...
    if (!connection) {
      throw new Error(
        'WebSocket connection is not initialized. Please establish a connection first.'
      )
    }
    ...
    return request.result.reduce(
      (entitys: EntityStateList, item: EntityStateItem) => {
