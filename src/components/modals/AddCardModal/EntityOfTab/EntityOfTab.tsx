...
interface EntityOfTabProps {
  entities: EntityTypes[]
  setSelectedEntity: (entity: EntityTypes) => void
}

function EntityOfTab({ entities, setSelectedEntity }: EntityOfTabProps) {
  const groupEntities = useMemo(() => {
    const groupedEntities: Record<
      string,
      { name: string; entities: EntityTypes[] }
    > = {}
    ...
    entities.forEach((entity: EntityTypes) => {
