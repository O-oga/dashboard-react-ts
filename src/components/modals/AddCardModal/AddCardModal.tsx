...
export const CardCreationDataContext = createContext<CardCreationData | null>(
  null
)

function AddCardModal({ isModalOpen, closeModal, spaceId }: AddCardModalProps) {
  ...
  const [tabsEntitys, setTabsEntitys] = useState<Record<EntityTypes, string[]>>(
  ...
  const getExistingTypes = (entitys: Record<string, string>) => {
    ...
    return Object.keys(entitys)
      ...
      .filter(type =>
        allowedTypesSet.has(type as (typeof ENTITY_TYPES)[number])
      ) //types from EntityTypes
      .sort()
  }
  ...
  const createCard = () => {
    ...
    addCard(spaceId, {
      id: Date.now(),
      title: title,
      entity: selectedEntity as EntityTypes,
      icon: selectedIcon,
      size: selectedSize,
      type: selectedTab as EntityTypes,
      state: '',
      card_data: [] as CardDataTypes[],
    })
    closeModal()
  }
          ...
          >
            <main className="add-card-content-container">
              <section className="left-container">
                <nav
                  className="add-card-tabs"
                  aria-label={t('addCard.entityTypeNavigation')}
                >
                  {tabs.length > 0 &&
                    tabs.map(tab => (
                      <button
                        key={tab}
                        className="button-text tab-button"
                ...
                <section
                  className="entity-selection-window"
                  aria-label={t('addCard.entitySelection')}
                >
                  {selectedTab && tabsEntitys[selectedTab] && (
                    <EntityOfTab
                      key={selectedTab}
                      entities={
                        (tabsEntitys[selectedTab] as EntityTypes[]) || []
                      }
            ...
            <footer className="footer-container">
              <form
                className="add-card-form footer-container"
                onSubmit={e => {
                  e.preventDefault()
                }}
              >
                ...
                <div className="add-card-buttons-container">
                  ...
                  <button
                    type="button"
                    className="add-card-cancel-button button-cancel"
                    onClick={closeModal}
                  >
                    {t('addCard.cancel')}
                  </button>
